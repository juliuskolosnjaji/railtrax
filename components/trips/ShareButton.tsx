...
export function ShareButton({ trip, onShare, onUnshare }: ShareButtonProps) {
  ...
  const shareUrl = trip.shareToken
    ? `${typeof window !== 'undefined' ? window.location.origin : ''}/trip/${trip.shareToken}`
    : null
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {[
                      ...
                      },
                      {
                        ...
                        href: `https://wa.me/?text=${encodeURIComponent('Schau dir meine Reise auf Railtrax an: ' + (shareUrl ?? ''))}`,
                      },
                    ].map(({ label, icon, href }) => (
                      <a
                        key={label}
