...
export async function exportTripAsImage(
  ...
  ctx.font = 'bold 11px system-ui, -apple-system, sans-serif'
  ctx.textAlign = 'right'
  ctx.fillText('RAILTRIPPER', PANEL_X + PANEL_W - PAD, 614)

  // Download
  const link = document.createElement('a')
  link.download = `railtripper-${trip.title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}.png`
...
export async function exportTripAsPdf(
  ...
  pdf.text(
    `Generated by Railtripper · ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}`,
    M, PH - 5,
  )
  pdf.setFont('helvetica', 'bold')
  pdf.setTextColor(100, 116, 139)
  pdf.text('RAILTRIPPER', PW - M, PH - 5, { align: 'right' })

  pdf.save(`railtripper-${trip.title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}.pdf`)
}
